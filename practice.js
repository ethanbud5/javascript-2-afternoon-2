...
function looper(family){
  family.map(person=>alert(person));
}
...
function divider(numbersArray){
  let oddArray = [];
  let evenArray = numbersArray.filter(num=>{
    if(num%2===0){
      return num;
    }
    else{
...
function finder(arr){
  ...
  let isFound = (arr.indexOf(randomNumber) === -1)?false:true;
...
function removeItem(myGroceryList,item){
  if(myGroceryList){
    if(item){
      let itemToRemove = myGroceryList.indexOf(item);
      if(itemToRemove===-1){
      }
      else{
...
devMountainEmployees.map((employee,i)=>{
